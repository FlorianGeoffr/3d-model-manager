...
async def create_model(
    ...
    return await library.build_model_detail(db, model)
...
async def get_model(slug: str, db: AsyncSession = Depends(get_db)) -> ModelDetail:
...
async def patch_model(
    slug: str, payload: ModelPatch, db: AsyncSession = Depends(get_db)
) -> ModelDetail:
    model = await library.get_model_by_slug(db, slug)
