...
from DB-backed settings rather than env vars -- at that point ``get_backend``
will read the active scheme/config from the DB instead of always defaulting
to ``local``.
"""
...
_BackendFactory = Callable[[Settings], StorageBackend]
...
@register("local")
def _build_local_backend(settings: Settings) -> StorageBackend:
    return LocalStorageBackend(settings.library_root)


def get_backend(settings: Settings, scheme: str = "local") -> StorageBackend:
    """Return the registered backend for ``scheme`` (``local`` by default)."""
    try:
        factory = _REGISTRY[scheme]
    except KeyError as e:
        raise StorageError(f"no storage backend registered for scheme {scheme!r}") from e
