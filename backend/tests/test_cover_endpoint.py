...
from app.models import Blob, Derivative, File, Model
...
async def test_set_cover_stores_blob_file_and_cover_hash(
    ...
    assert file_row.rel_path.startswith("_snapshots/cover-")
    assert file_row.rel_path.endswith(".png")
...
async def test_repost_replaces_cover_hash(authenticated_client: httpx.AsyncClient) -> None:
